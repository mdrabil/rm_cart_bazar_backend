...
export async function verifyPayment({
  ...
  if (!cid) {
    console.error(
      `[Payment][verifyPayment] Customer context missing — gatewayOrderId=${gatewayOrderId} ` +
        `transactionId=${transactionId || "(none)"} source=${source} ` +
        `sessionFound=${Boolean(session)} explicitCustomerId=${customerId || "(none)"}`
    );
  ...
  try {
    ...
    try {
      ...
      db.endSession();
      emitOrderUpdate({ type: "new", order });
      console.log(`[Payment][verifyPayment] SUCCESS orderId=${order._id} gatewayOrderId=${gatewayOrderId} source=${source} attempt=${attempt}`);
      return { success: true, orderId: order._id, order, user: updatedUser, payment };
    } catch (e) {
      await db.abortTransaction();
      db.endSession();
    ...
    console.warn(
      `[Payment][verifyPayment] not yet successful gatewayOrderId=${gatewayOrderId} attempt=${attempt} ` +
        `source=${source} reason=${err.message}`
    );
    if (payment) {
      payment.status = PAYMENT_STATUS.PENDING;
      payment.verificationAttempts = attempt;
...
export async function getCheckoutPage(sessionId, apiBaseUrl) {
  // Checkout-page validity is intentionally a short, separate window from
  // SESSION_TTL_MS (which now covers the full retry chain, ~2hrs) — a
  // customer shouldn't be able to resume paying into an hours-old,
  // possibly-stale session. This only gates rendering the pay page; it does
  // not delete the underlying record needed by the retry worker.
  const session = await PaymentCheckoutSession.findOne({
    sessionId,
    status: { $in: [CHECKOUT_SESSION_STATUS.PENDING, CHECKOUT_SESSION_STATUS.PROCESSING] },
  });
  if (!session) throw new Error("Checkout session expired or not found");
  if (Date.now() - session.createdAt.getTime() > CHECKOUT_PAGE_VALID_MS) {
...
export async function handleReturn(sessionId, req) {
  console.log(`[Payment][handleReturn] sessionId=${sessionId}`);
  // Deliberately no expiresAt filter here: this runs AFTER the customer has
  // already been through the gateway (possibly a real captured payment). The
  // old `expiresAt: { $gt: new Date() }` filter used the same 30-minute
  // window as checkout-page validity, so a customer who took slightly over
  // 30 minutes to complete payment — but did pay — would get told "session
  // expired" here instead of being verified and fulfilled. We still bound it
  // to SESSION_TTL_MS (the retry-worker's window, ~2hrs) purely so ancient,
  // truly-abandoned sessionIds don't resolve forever.
  const session = await PaymentCheckoutSession.findOne({ sessionId });
  if (!session || Date.now() - session.createdAt.getTime() > SESSION_TTL_MS) {
    console.warn(`[Payment][handleReturn] no live session for sessionId=${sessionId}`);
    throw new Error("Checkout session expired or not found");
  }
...
export async function runScheduledVerifications() {
  ...
  if (due.length) {
    console.log(`[Payment][RetryWorker] ${due.length} payment(s) due for verification`);
  }
  ...
  for (const p of due) {
    ...
    if (!session) {
      console.warn(
        `[Payment][RetryWorker] no session found for paymentId=${p._id} ` +
          `checkoutSessionId=${p.checkoutSessionId} gatewayOrderId=${p.gatewayOrderId} — verification will likely fail`
      );
    }
