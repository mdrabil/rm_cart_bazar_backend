  ...
  return {
    ...
    async createPayment({ amount, sessionId, customer }) {
      ...
      const form = {
        ...
        txnid,
        amount: amountStr,
        productinfo: "MR Brand Order",
        firstname: customer?.name || "Customer",
        email: customer?.email || "customer@example.com",
        phone: customer?.phone || "9999999999",
        hash: buildHash({ txnid, amount: amountStr, productinfo: "MR Brand Order", firstname: customer?.name || "Customer", email: customer?.email || "customer@example.com", phone: customer?.phone }),
      };
