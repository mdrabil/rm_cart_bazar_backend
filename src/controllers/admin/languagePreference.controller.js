...
export const getAdminLanguagePreferences = async (req, res) => {
  ...
  $unwind: {
    ...
    const countResult = await LanguagePreferenceModel.aggregate(countPipeline);
    const total = countResult[0]?.total || 0;

    const summary = await LanguagePreferenceModel.aggregate([
      {
        $group: {
          _id: null,
          totalUsers: { $sum: 1 },
    ...
    const preferences = await LanguagePreferenceModel.aggregate(pipeline);
...
export const toggleLanguagePreferenceStatus = async (req, res) => {
  try {
    ...
    const preference = await LanguagePreferenceModel.findById(id).populate(
      "customerId",
